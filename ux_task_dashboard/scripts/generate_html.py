...
<body class="min-h-screen text-slate-200">
        ...
        <div class="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
            <div class="flex flex-row items-center justify-between gap-4">
                <div>
                    ...
                    <h1 class="text-xl sm:text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-brand-400 outfit-font tracking-tight mt-1">
                ...
                <div class="flex items-center shrink-0 gap-3">
                    <button onclick="syncWithClickUp()" id="sync-btn" class="px-3 py-1.5 bg-slate-900/60 hover:bg-brand-900/10 border border-slate-850 hover:border-brand-500/30 text-slate-400 hover:text-brand-400 text-xs font-semibold rounded-lg shadow-sm transition-all flex items-center gap-1.5 cursor-pointer">
    ...
    <div class="max-w-8xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <aside class="lg:col-span-1">
                <div class="glass-effect p-4 rounded-xl sticky top-36 overflow-y-auto max-h-[calc(100vh-11rem)] border border-brand-900/10 shadow-xl shadow-black/20">
            ...
            <main class="lg:col-span-3">
                ...
                <div class="flex items-center justify-between mb-4 pb-2 border-b border-slate-900">
                    ...
                    <div class="flex items-center gap-2">
                        <button onclick="expandAll()" class="px-3 py-1.5 bg-slate-900/60 hover:bg-brand-900/10 border border-slate-850 hover:border-brand-500/30 text-slate-350 hover:text-slate-100 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5">
                            <i class="fa-solid fa-folder-open text-[10px] text-brand-500/80"></i> Expand All
                        </button>
                        <button onclick="collapseAll()" class="px-3 py-1.5 bg-slate-900/60 hover:bg-slate-800 border border-slate-850 hover:border-slate-755 text-slate-350 hover:text-slate-205 text-xs font-semibold rounded-lg transition-all flex items-center gap-1.5">
    ...
    <script>
        ...
        function renderMainNode(node, level = 0) {{
            ...
            let containerClass = "rounded-xl border shadow-md transition-all duration-300 ";
            ...
            if (level === 0) {{
                ...
                borderStyle = "border-l-4 border-l-brand-500/80";
            }} else {{
            ...
            function highlightText(text) {{
                ...
                return text.replace(regex, '<mark class="bg-brand-500/25 text-brand-200 border-b border-brand-500/80 px-0.5 rounded">$1</mark>');
            }}
            ...
            const timeBadge = totalEst 
                ? `<span class="text-xs text-slate-455 flex items-center gap-1 bg-slate-900/60 px-2 py-1 rounded border border-slate-855/80 whitespace-nowrap">
                       <i class="fa-solid fa-clock text-brand-500/80"></i> ${{level === 0 ? totalEst : selfEst || totalEst}}
                   </span>`
                : '';

            // Subtask count badge
            const subCountBadge = hasSub
                ? `<span class="text-[10px] text-slate-455 bg-brand-900/10 px-2 py-0.5 rounded-full border border-brand-800/10 whitespace-nowrap flex items-center gap-1 font-medium">
